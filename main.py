...
class DiffusionModel():
    ...
    def __init__(self, T, model: nn.Module, device, dim=2):
        ...
        self.betas = (torch.sigmoid(torch.linspace(-18, 10, T))
                      * (3e-1 - 1e-5) + 1e-5).to(device)
    ...
    def forward_process(self, x0, t):
        ...
        m1 = torch.sqrt(self.alphas_bar[t-1]) * \
            self.betas[t] / (1 - self.alphas_bar[t])
        m2 = torch.sqrt(
            self.alphas[t]) * (1 - self.alphas_bar[t-1]) / (1 - self.alphas_bar[t])
        mu_q = m1 * x0 + m2 * xt
    ...
    def get_loss(self, x0):
        ...
        mu_p, sigma_p, _ = self.reverse_process(xt.float(), t)
...
def plot(model, file_name, device):
    ...
    data = [x0.cpu(), model.forward_process(x0, 20)[-1].cpu(),
            model.forward_process(x0, 40)[-1].cpu()]
    for i in range(3):

        plt.subplot(2, 3, 1+i)
        plt.scatter(data[i][:, 0].data.numpy(), data[i]
                    [:, 1].data.numpy(), alpha=0.1, s=1)
        plt.xlim([-2, 2])
        plt.ylim([-2, 2])
